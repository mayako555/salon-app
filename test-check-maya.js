...
(async () => {
  try {
    ...
    const snap = await db.collection('staff_profiles').where('email', '==', 'mayainny5@gmail.com').get();
    if (snap.empty) {
      console.log('No Firestore doc found');
    } else {
      snap.forEach(doc => {
        console.log('Firestore Doc ID:', doc.id);
        console.log('Firestore UID:', doc.data().uid);
        if (doc.data().uid !== user.uid) {
          console.log('UID MISMATCH! Fixing...');
          doc.ref.update({ uid: user.uid });
          console.log('Fixed!');
        }
      });
    }
