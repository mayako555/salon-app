    ...
    rawShifts.forEach(s => {
      ...
      if (!existing) {
        ...
        const isHolidayType = (type: string) => 
          type === 'holiday' || type === 'paid_leave' || type === 'half_paid_leave' ||
  ...
  const renderCompactShift = (shift: any) => {
    ...
    return (
      ...
      >
        {shift.type === 'holiday' ? (
        ...
        ) : shift.type === 'paid_leave' || shift.type === 'half_paid_leave' ? (
          <div className={`font-bold p-1 rounded h-full flex flex-col justify-center ${shift.request_id ? 'text-emerald-700 bg-emerald-50 border border-emerald-200' : 'text-amber-600 bg-amber-50'}`}>
            {shift.request_id ? (shift.type === 'half_paid_leave' ? "半休(希望)" : "有休(希望)") : (shift.type === 'half_paid_leave' ? "半休" : "有休")}
          </div>
