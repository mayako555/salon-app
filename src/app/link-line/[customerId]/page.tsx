...
export default function LinkLinePage() {
  ...
  const configuredLiffId = searchParams.get("liffId");

  useEffect(() => {
    const initLiff = async () => {
      try {
        // LIFF ID is public configuration. Store-specific links always carry the
        // tenant-scoped value generated by the server and never use another
        // store's shared fallback.
        const liffId = configuredLiffId || (!store ? process.env.NEXT_PUBLIC_LIFF_ID : undefined);
    ...
    initLiff();
  }, [configuredLiffId, customerId, store]);
