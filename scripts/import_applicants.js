...
// Initialize Firebase Admin (adjust the credential method based on your setup)
if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.applicationDefault(), // Uses GOOGLE_APPLICATION_CREDENTIALS or default local creds
...
async function run() {
  ...
  for (const app of applicants) {
    app.created_at = admin.firestore.FieldValue.serverTimestamp();
    app.updated_at = admin.firestore.FieldValue.serverTimestamp();
    
    // Convert source text to match enum
    ...
    await db.collection(APPLICANTS_COLLECTION).add(app);
    count++;
    console.log(`Added ${app.name}`);
  }
  console.log(`Successfully added ${count} applicants!`);
}
