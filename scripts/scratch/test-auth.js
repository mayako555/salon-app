...
(async () => {
  try {
    const user = await admin.auth().getUserByEmail('suzuka110607@icloud.com');
    console.log('Firebase Auth User Found:', user.uid);
    
    // Update the password directly here just in case!
    await admin.auth().updateUser(user.uid, {
      password: "0000_salon"
    });
    console.log('Password forcibly reset to 0000_salon');
